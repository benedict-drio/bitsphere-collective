/**
 * BitSphere Collective - Chainhook Monitoring Setup
 * 
...
// Configuration
const CONTRACT_ADDRESS = 'SP3P2G9ZK7B309EGAM9QAM143YGDNBGGQAXDAS1GW.bitsphere-collective';
...
/**
 * Set up all monitoring chainhooks for BitSphere Collective
 */
async function setupMonitoring() {
  console.log('🔍 Setting up chainhook monitoring for BitSphere Collective...\n');

  try {
    // 1. Monitor member joins
    const joinHook = await client.registerChainhook({
      name: 'BitSphere - Member Joins',
      chain: 'stacks',
      network: 'mainnet',
      version: 1,
      filters: {
    ...
    const executionHook = await client.registerChainhook({
      name: 'BitSphere - Proposal Executions',
      chain: 'stacks',
      network: 'mainnet',
      version: 1,
      filters: {
    ...
    const allCallsHook = await client.registerChainhook({
      name: 'BitSphere - All Contract Calls',
      chain: 'stacks',
      network: 'mainnet',
      version: 1,
      filters: {
